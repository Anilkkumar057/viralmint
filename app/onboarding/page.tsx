...
export default function OnboardingPage() {
  ...
  const completeOnboarding = async () => {
    if (!niche || !platform || !style || !goal) return;
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_14%_8%,rgba(126,242,194,0.30),transparent_26%),radial-gradient(circle_at_88%_12%,rgba(245,199,107,0.26),transparent_24%),radial-gradient(circle_at_50%_96%,rgba(255,107,95,0.12),transparent_30%),linear-gradient(135deg,#fffaf2_0%,#fff7e8_45%,#f7fff9_100%)] px-6 py-10 text-black">
      <div className="mx-auto max-w-4xl rounded-[3rem] border border-black/10 bg-white/60 p-8 shadow-[0_25px_85px_rgba(126,242,194,0.14)] backdrop-blur-xl md:p-12">
        ...
        <p className="mt-6 max-w-2xl text-sm leading-8 text-black/55 md:text-base">
          Your AI generation system will adapt to your platform,
          audience psychology, storytelling energy, and creator goals.
        </p>
        ...
        <button
          ...
          disabled={loading}
          className="mt-16 rounded-full bg-black px-10 py-5 text-xs uppercase tracking-[0.3em] text-white shadow-[0_12px_38px_rgba(126,242,194,0.24)] transition-all duration-500 hover:scale-[1.03] hover:shadow-[0_16px_50px_rgba(126,242,194,0.38)] disabled:opacity-40"
        >
          {loading ? "Preparing..." : "Enter Creator Studio"}
        </button>
      </div>
    </main>
  );
          ...
          <button
            ...
            className={
              selected === item
                ...
                : "rounded-full border border-black/10 bg-white/70 px-6 py-3 text-xs uppercase tracking-[0.25em] text-black/50 transition-all duration-500 hover:bg-black hover:text-white"
            }
          >
            {item}
          </button>
